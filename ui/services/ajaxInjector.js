...
export default class Service extends ServiceInjector {
  ...
  responseError = (resp) => {
    ...
    let errBody;
    if (resp.data) {
      errBody = resp.data.reason;
    } else {
      errBody = this.$injector.get('$translate').instant('ajax.networkError');
    }
    this.$injector.get('dialogs').error(
      this.$injector.get('$translate').instant(`ajax.${resp.config.method.toLowerCase()}FailMsg`),
      errBody
    );
    if (resp.config.method !== 'GET') {
      this.addN(-1);
